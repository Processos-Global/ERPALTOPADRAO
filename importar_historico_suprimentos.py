...
ARQUIVO_PADRAO = "CHECK LIST - REDE ALTO PADRÃO  (1).xlsx"


def configurar_django():
    """
    Descobre automaticamente o DJANGO_SETTINGS_MODULE definido no manage.py
    e inicializa o Django antes de importar qualquer model/app.
    """
    manage_py = BASE_DIR / "manage.py"

    if not manage_py.exists():
        raise RuntimeError(
            f"manage.py não encontrado em: {manage_py}"
        )

    conteudo = manage_py.read_text(
        encoding="utf-8",
        errors="ignore",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Importa o histórico legado de suprimentos."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Valida a importação sem gravar alterações no banco.",
    )
    ...
    parametros = [
        "importar_historico_suprimentos",
        str(caminho),
    ]
